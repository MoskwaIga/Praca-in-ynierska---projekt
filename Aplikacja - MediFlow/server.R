...
server <- function(input, output, session) {
  ...
  output$attrRatePlot <- renderHighchart({
    ...
    if (input$gender_split) { # Warunek podziału na płeć
      ...
      attrRateData <- data %>%
        group_by(across(all_of(input$filter_var)), Gender) %>%
        summarise(AttritionRate = round((sum(Attrition == "Yes") / n()) * 100, 2)) %>%
        ungroup()
      ...
      hchart(attrRateData, "column", hcaes(x = !!sym(input$filter_var), y = AttritionRate,
        ...
        hc_legend(title = list(text = "Gender", align = "center")) %>%  
  ...
  output$attritionCountPlot <- renderHighchart({
    ...
    if (input$gender_split) {
      attritionCount <- data %>%
        ...
        group_by(!!sym(input$filter_var), Gender) %>%
        summarise(Count = n(), .groups = 'drop')
      
      ## W Y K R E S --------
      hchart(attritionCount, "column", hcaes(x = !!sym(input$filter_var), y = Count, group = Gender)) %>%
  ...
  output$monthlyIncomeYesPlot <- renderHighchart({
    ...
    selected_var <- if (input$filter_var == "IncomeGroup") previous_var() else input$filter_var 
    
    plot_data <- data %>%
      filter(Attrition == "Yes") %>%
      select(MonthlyIncome, !!sym(selected_var), Gender) %>% 
      group_by(!!sym(selected_var), Gender)
    
    if (input$gender_split) {
      # Jeśli zaznaczony jest podział na płeć
      
      ## W Y K R E S ----------
      hchart(plot_data, "scatter", hcaes(x = !!sym(selected_var), y = MonthlyIncome, 
                                         group = interaction(Gender, !!sym(selected_var)))) %>%
        ...
        hc_legend(title = list(text = paste("Gender & ", selected_var), align = "center")) %>%
        hc_colors(c("#128870", "#333333")) %>%  
        hc_exporting(enabled = TRUE, 
                     buttons = list(contextButton = list(menuItems = c("viewFullscreen", "separator",
                                                                       "downloadPNG", "downloadJPEG", 
  ...
  generatePlot <- function(gender) {
    filteredData <- data %>%
      filter(Attrition == "Yes", Gender == gender) %>%
      group_by(OverTime, !!sym(input$filter_var)) %>%
      summarise(Count = n(), .groups = 'drop') %>%
      ungroup() %>%
      group_by(!!sym(input$filter_var)) %>%
  ...
  observeEvent(input$gender_split, {
    ...
    if (input$gender_split) {
      ...
      output$femalePlot <- renderHighchart({
        femaleData <- generatePlot("Female")
        
        ## W Y K R E S --------
        hchart(femaleData, "column", hcaes(x = !!sym(input$filter_var), y = Percentage, group = OverTime)) %>%
          ...
          hc_legend(title = list(text = "OverTime", align = "center")) %>%  
          hc_exporting(enabled = TRUE,
                       buttons = list(contextButton = list(menuItems = c("viewFullscreen", "separator", 
                                                                         "downloadPNG", "downloadJPEG", 
                                                                         "downloadPDF", "downloadSVG"))))
      ...
      output$malePlot <- renderHighchart({
        maleData <- generatePlot("Male")
        
        ## W Y K R E S --------
        hchart(maleData, "column", hcaes(x = !!sym(input$filter_var), y = Percentage, group = OverTime)) %>%
      ...
      output$overTimePlot <- renderHighchart({
        ...
        ## W Y K R E S --------
        hchart(overTimeData, "column", hcaes(x = !!sym(input$filter_var), y = Percentage, group = OverTime)) %>%
  ...
  output$satisfactionRotationPlot <- renderHighchart({
    ...
    if (input$gender_split) {
      ...
      avgWorkRate <- data %>%
        filter(Attrition == "Yes") %>%
        group_by(!!sym(input$filter_var), Gender) %>%
        summarise(AvgWorkRate = round(mean(JobSatisfaction), 2), .groups = 'drop')
      
      ## W Y K R E S -----------
      hchart(avgWorkRate, "column", hcaes(x = !!sym(input$filter_var), y = AvgWorkRate, group = Gender)) %>%
  ...
  output$genderAttritionPlot <- renderHighchart({
    ...
    ## W Y K R E S -------
    hchart(genderAttrition, "pie", hcaes(y = Percentage, name = Gender)) %>%
  ...
  output$worklifeBalanceHeatmapPlot <- renderHighchart({
    ...
    worklifeBalance_data <- data %>%
      filter(Attrition == "Yes") %>% 
      group_by(Gender, !!sym(input$filter_var)) %>%
      summarise(AvgWorkLifeBalance = round(mean(WorkLifeBalance), 2)) %>%
      ungroup()
    
    ## W Y K R E S ---------------
    hchart(worklifeBalance_data, type = "heatmap", 
           hcaes(x = Gender, y = !!sym(input$filter_var), value = AvgWorkLifeBalance)) %>%
