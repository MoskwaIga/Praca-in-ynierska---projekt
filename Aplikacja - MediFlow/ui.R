    ...
    tabItem(
      ...
      tabsetPanel(id = "tabs",
                  tabPanel("Rotacja",
                          ...
                          fluidRow(
                            column(6, highchartOutput("attrRatePlot", height = "500px")),
                            column(6, highchartOutput("attritionCountPlot", height = "500px"))
                          )),
                  tabPanel("Miesięczne Dochody",
                          fluidRow(
                            column(6, highchartOutput("monthlyIncomeYesPlot", height = "500px")),
                            column(6, highchartOutput("monthlyIncomeNoPlot", height = "500px"))
                          )),
                  tabPanel("Nadgodziny",
                           fluidRow(
                             conditionalPanel(
                               condition = "input.gender_split == false",
                               column(12, highchartOutput("overTimePlot", height = "500px"))
                             ),
                             
                             conditionalPanel(
                               condition = "input.gender_split == true",
                               column(6, highchartOutput("femalePlot", height = "500px")),
                               column(6, highchartOutput("malePlot", height = "500px"))
                             )
                           )),
                  tabPanel("Satysfakcja z pracy",
                           fluidRow(
                             column(6, highchartOutput("satisfactionNoRotationPlot", height = "500px")),
                             column(6, highchartOutput("satisfactionRotationPlot", height = "500px"))
                           )),
                  tabPanel("Pozostałe",
                           fluidRow(
                             column(6, highchartOutput("genderAttritionPlot", height = "500px")),
                             column(6, highchartOutput("worklifeBalanceHeatmapPlot", height = "500px"))
                           ))
